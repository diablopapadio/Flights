...
def get_top_10_longest_paths(source_code):
    ...
    # Obtener información detallada de los aeropuertos en los caminos mínimos
    detailed_airport_info = []
    for code, _ in top_10_longest_paths:
        airport_info = df[df['Source Airport Code'] == code].iloc[0]
        shortest_path = nx.shortest_path(G, source=source_code, target=code, weight='weight')
        total_distance = sum(G[shortest_path[i]][shortest_path[i + 1]]['weight'] for i in range(len(shortest_path) - 1))
        detailed_airport_info.append((code,
                                      airport_info['Source Airport Name'],
                                      airport_info['Source Airport City'],
                                      airport_info['Source Airport Country'],
                                      airport_info['Source Airport Latitude'],
                                      airport_info['Source Airport Longitude'],
                                      shortest_path,
                                      total_distance))

    return detailed_airport_info
    ...
    if 'submit-val' in changed_id:
        ...
        if info_type == 'airports':
            ...
            info_output = html.Div([
                ...
                html.Table([
                    html.Tr([html.Th('Código'), html.Th('Nombre'), html.Th('Ciudad'), html.Th('País'),
                             html.Th('Latitud'), html.Th('Longitud'), html.Th('Recorrido'), html.Th('Distancia (km)')]),
                    *[html.Tr([html.Td(code), html.Td(name), html.Td(city), html.Td(country),
                                html.Td(lat), html.Td(lon), html.Td(" -> ".join(path)), html.Td(distance)]) 
                      for code, name, city, country, lat, lon, path, distance in top_10_longest_paths]
                ])
            ])
            ...
            lats = []
            lons = []
            for code, _, _, _, lat, lon, _, _ in top_10_longest_paths:
